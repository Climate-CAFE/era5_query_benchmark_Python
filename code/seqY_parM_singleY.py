...
if __name__ == '__main__':
    ...
    for i in range(len(variable_list)):
        ...
        # benchmark the task
        result = timeit(lambda: give_input(variables_repeat, query_months_str), setup='from __main__ import give_input', number=1)
        # report the result
        print('Time: {} seconds'.format(result))
        benchmarks.append(result)

    benchmarks_parallel = pd.DataFrame(benchmarks)
    benchmarks_parallel.to_csv('D:\\CAFE_DATA_MANAGEMENT\\ERA5_Python\\downloading_benchmark\\ERA5_Out_Parallel\\benchmarks_parallel.csv')
